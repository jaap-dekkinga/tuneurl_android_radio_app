...
object CollectionHelper {
    ...
    fun setStationImageWithRemoteLocation(
        ...
        collection.stations.forEach { station ->
            ...
            if (station.remoteImageLocation.substringAfter(":") == remoteFileLocation.substringAfter(
                ...
                station.imageURL = FileHelper.saveStationImage(
                    context,
                    station.uuid,
                    tempImageFileUri.toUri(),
    ...
    fun saveCollection(context: Context, collection: Collection, async: Boolean = true,canupdate: Boolean=false): Date {
        if (!canupdate)return Calendar.getInstance().time
        Log.v(
            TAG,
...
//            setMediaUri("https://stream.live.vc.bbcmedia.co.uk/bbc_world_service".toUri())
        }.build()
        // build MediaMetadata
        val mediaMetadata = MediaMetadata.Builder().apply {
            ...
            if (station.imageURL.isNotEmpty() && station.imageURL.startsWith("file://")) {
                //setArtworkUri(station.image.toUri())
                setArtworkData(station.imageURL.toUri().toFile().readBytes(), MediaMetadata.PICTURE_TYPE_FRONT_COVER)
            } else {
...
//            setUri("https://stream.live.vc.bbcmedia.co.uk/bbc_world_service".toUri())
        }.build()
    }
